...
create_deb_package() {
  ...
  mkdir -p zed-deb-pkg/usr/bin
  mkdir -p zed-deb-pkg/usr/lib/zed
  mkdir -p zed-deb-pkg/usr/share/applications
  mkdir -p zed-deb-pkg/usr/share/icons/hicolor/512x512/apps
...
EOF
  ...
  # Copy application files from the extracted directory
  echo "Copying application files to package"
  ls -la app_extract/
  cp -r app_extract/* zed-deb-pkg/usr/lib/zed/ || { echo "Error copying files from app_extract to package"; exit 1; }
  
  # Create launcher script
  cat > zed-deb-pkg/usr/bin/zed-preview << EOF
#!/bin/sh
exec /usr/lib/zed/bin/zed "\$@"
EOF
  chmod +x zed-deb-pkg/usr/bin/zed-preview
  
  # Create desktop file
  cat > zed-deb-pkg/usr/share/applications/zed-preview.desktop << EOF
[Desktop Entry]
Name=Zed Preview
Comment=High-performance, multiplayer code editor
Exec=/usr/bin/zed-preview %F
Icon=zed-preview
Terminal=false
Type=Application
...
EOF
  ...
  # Extract icon from app if available, or use a placeholder
  if [ -f "app_extract/share/icons/hicolor/512x512/apps/zed.png" ]; then
    cp "app_extract/share/icons/hicolor/512x512/apps/zed.png" zed-deb-pkg/usr/share/icons/hicolor/512x512/apps/zed-preview.png
  fi
